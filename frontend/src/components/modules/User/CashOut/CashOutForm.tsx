...
const calcFee = (amount: number) => {
  ...
  return Math.ceil(amount * 0.0185);
};
...
const CashOutForm = () => {
  ...
  useEffect(() => {
    ...
    if (state.success) {
      toast.success(state.message ?? "Cash out successful!");
    }
  }, [state]);
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start gap-2.5 rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 px-4 py-3">
            ...
            <p className="text-xs text-blue-700 dark:text-blue-300 leading-relaxed">
              Enter your agent&apos;s phone number. The agent must be registered and active.
            </p>
          </div>

          {/* Agent Phone */}
          <Field>
            <FieldLabel htmlFor="agentPhone">Agent Phone Number</FieldLabel>
            <div className="relative">
              ...
              <Input
                ...
                name="agentPhone"
                type="tel"
                placeholder="Agent's 01XXXXXXXXX"
                className="pl-10"
                maxLength={11}
              />
            </div>
            <InputFieldError field="agentPhone" state={state} />
          </Field>
          ...
          {numericAmount > 0 && (
            <Card className="bg-muted/40 border-dashed">
              <CardContent className="p-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between text-muted-foreground">
                  <span>Fee (1.85%)</span>
                  <span className="tabular-nums text-orange-500">৳ {fee.toLocaleString()}</span>
                </div>
                <Separator />
                ...
                <p className="text-xs text-muted-foreground pt-1">
                  Agent receives: <span className="font-medium text-foreground">৳ {numericAmount.toLocaleString()}</span>
                </p>
              </CardContent>
            </Card>
          )}
          ...
          <Button
            ...
            className="w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold h-11 rounded-xl gap-2 cursor-pointer"
          >
