...
interface AdminDashboardContentProps {
  ...
  initialStats?: IResponse<IAdminStats>;
}
  ...
  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 ">
    {[
      ...
      },
      {
        ...
        value: stats.pendingAgentRequests,
        icon: ClipboardList,
      ...
      },
      {
        ...
        value: stats.totalSystemCommission,
        icon: TrendingUp,
        ...
        <Card key={item.label}>
          <CardContent className="p-4">
            ...
            <p className={`text-xl font-bold tabular-nums ${item.color}`}>
              {item.prefix}
              {item.format ? Number(item.value).toLocaleString() : item.value}
            </p>
          </CardContent>
        </Card>
      );
...
const AdminDashboardContent = ({
  ...
  // fetch realtime data admin stats
  const realtimeStatsFetch = useCallback(() => getAdminDashboardStats(), []);
  ...
  // stats
  const { data: realtimeStats } = useRealtimeWallet({
    fetchFn: realtimeStatsFetch,
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-gradient-to-br from-slate-800 to-slate-900 dark:from-slate-900 dark:to-black p-6 text-white">
        <div className="flex items-center justify-between">
          ...
          <Badge className="bg-white/10 text-white border-0 hover:bg-white/10">
            Admin
          </Badge>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {QUICK_LINKS.map((link) => {
          ...
          return (
            ...
            >
              <div
                className={`h-10 w-10 rounded-xl flex items-center justify-center ${link.color}`}
              >
